    ...
    public class UserService : IUserService
    {
        ...
        public string SendForgotPasswordMail(string email)
        {
           IUserRepository userRepo = new UserRepository();
                ...
                if (isValidemail)
                {
                    try
                    {
                        ...
                        if (user != null)
                        {
                            ...
                            message = "Your reset password email has been sent to '" + email + "'.";
                        }
                    }
                    catch (Exception ex)
                    {
                        //message = ex.Message;
                        message = "Your reset password email has not been sent to '" + email + "'.";
                    }
            ...
            {
                message = "Email address can not be set Empty";
            }
            return message;
        }
        ...
        public void UpdatePassword(string newPassword, string newConfirmPassword, int userId)
        {
            IUserRepository userRepo = new UserRepository();
            string message = "";
